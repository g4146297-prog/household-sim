...
# --- ページ設定 ---
st.set_page_config(
    page_title="将来家計シミュレーション (東京・持家版)",
    layout="wide",
...
EDUCATION_COSTS = {
    '【A】公立中心(塾しっかり)': [
        ...
        # 小学校(公立+習い事)
        35, 35, 35, 40, 45, 50, 
        # 中学校(公立+高校受験塾:中3は講習費増)
        60, 60, 80, 
        # 高校(公立+大学受験予備校)
        60, 70, 90, 
        # 大学(国公立:入学金+授業料) ※仕送りは別途計算
        90, 55, 55, 55, 0
    ],
    '【B】中高公立・私大文系': [
        10, 10, 10, 25, 25, 25, 
    ...
    '【D】高校から私立(文系大)': [ 
        ...
        35, 35, 35, 40, 45, 50, 
        60, 60, 80,
        # 高校私立(東京平均:授業料実質無償化でも施設費・修学旅行等かかる)
        100, 100, 110, 
        135, 105, 105, 105, 0
    ],
    '【E】高校から私立(理系大)': [
...
REARING_COSTS = {
    '【A】標準プラン': [
        ...
        # 高校生 (交際費・食費大人並)
        140, 140, 140, 
        # 大学生 (国民年金・交際費等 ※学費・住居費別)
        100, 100, 100, 100, 0
    ],
    '【B】ゆとりプラン': [
        100, 100, 100, 
        110, 110, 110, 
        120, 120, 120, 130, 130, 140, 
        150, 150, 150, 
        160, 160, 160, 
        150, 150, 150, 150, 0
    ],
...
# 生活費プリセット (固定資産税・車維持費・住居費を除く純粋な生活費)
# 食費、光熱費、通信、日用品、被服、夫婦の娯楽など
LIVING_PRESETS = {
    '【A】節約 (月30万)': 360,
    '【B】標準 (月38万)': 456, 
    '【C】ゆとり (月48万)': 576,
...
# --- サイドバー設定 ---
st.sidebar.title("🛠️ 設定 (東京23区外・持家)")
...
c1_boarding = st.sidebar.checkbox("第1子 大学は下宿(仕送り)する", value=False)
...
if has_child2:
    ...
    c2_boarding = st.sidebar.checkbox("第2子 大学は下宿(仕送り)する", value=False)
else:
    c2_year, c2_month = None, None
    c2_edu = None
    c2_boarding = False

# 下宿代の設定（チェックが入った場合のみ有効）
boarding_cost_yearly = 0
if c1_boarding or c2_boarding:
    st.sidebar.markdown("##### 📦 下宿・仕送り費用")
    boarding_cost_yearly = st.sidebar.number_input(
        "年間仕送り額 (家賃+生活費補助)", 
        value=150, 
        step=10, 
        help="地方での一人暮らし想定。例: 家賃6万+生活費6.5万=12.5万/月×12ヶ月=150万"
    )
...
st.sidebar.markdown("---")
living_preset_key = st.sidebar.selectbox("生活費プリセット(住居費別)", list(LIVING_PRESETS.keys()), index=1)
living_cost_base = st.sidebar.number_input("年間生活費 (万円)", value=LIVING_PRESETS[living_preset_key], step=10)

fixed_cost_housing = st.sidebar.number_input(
    "固定資産税・維持費 (年額)", 
    value=19.2, 
    step=0.1, 
    help="固定資産税(4.8万×4回)など。車維持費は0円設定。"
)
...
def get_boarding_cost(age, is_boarding, cost_per_year):
    # 18, 19, 20, 21歳 (大学4年間)
    if is_boarding and (18 <= age <= 21):
        return cost_per_year
    return 0

# --- メインコンテンツ ---
st.title("将来家計シミュレーション 📊")
st.caption("東京23区外・持家・車なし想定")
...
df['第1子養育費'] = df['第1子年齢'].apply(lambda x: get_cost(x, edited_rear.loc['【A】標準プラン'].tolist()) if x >= 0 else 0)
df['第1子仕送り'] = df['第1子年齢'].apply(lambda x: get_boarding_cost(x, c1_boarding, boarding_cost_yearly) if x >= 0 else 0)

if has_child2:
    df['第2子教育費'] = df['第2子年齢'].apply(lambda x: get_cost(x, edited_edu.loc[c2_edu].tolist()) if x >= 0 else 0)
    df['第2子養育費'] = df['第2子年齢'].apply(lambda x: get_cost(x, edited_rear.loc['【A】標準プラン'].tolist()) if x >= 0 else 0)
    df['第2子仕送り'] = df['第2子年齢'].apply(lambda x: get_boarding_cost(x, c2_boarding, boarding_cost_yearly) if x >= 0 else 0)
else:
    df['第2子教育費'] = 0
    df['第2子養育費'] = 0
    df['第2子仕送り'] = 0
...
df['仕送り合計'] = df['第1子仕送り'] + df['第2子仕送り']
df['教育・養育・仕送り小計'] = df['教育費合計'] + df['養育費合計'] + df['仕送り合計']

# 生活費計算 (インフレ考慮)
# 基礎生活費(インフレあり) + 固定資産税(インフレなし想定だが、念のため維持費としてインフレさせるか選択余地あり。ここではインフレさせる)
df['生活費(変動)'] = living_cost_base * (1 + inflation_rate) ** df['経過年数']
df['固定費(住宅)'] = fixed_cost_housing # 固定資産税は評価替えで変わるが、維持費増もあるので一定または微増。今回は一定とする。
df['生活費総合'] = df['生活費(変動)'] + df['固定費(住宅)']
...
for i, year in enumerate(years):
    ...
    spending_val = (df['教育・養育・仕送り小計'].iloc[i] + df['生活費総合'].iloc[i]) * 10000 + annual_payment
...
df['収支'] = df['世帯収入'] - (df['教育・養育・仕送り小計'] + df['生活費総合'] + df['ローン返済'])
...
with col_s1:
    total_edu_rear = df['教育・養育・仕送り小計'].sum()
    st.metric("教育・養育・仕送り総額", f"{total_edu_rear:,.0f} 万円", f"教育:{df['教育費合計'].sum():,.0f} / 仕送り:{df['仕送り合計'].sum():,.0f}")
...
display_cols = ['西暦', '第1子年齢', '世帯収入', '教育費合計', '仕送り合計', '生活費総合', 'ローン返済', '収支', '総資産', 'ローン残高']
...
if st.button("AIに診断してもらう"):
    if not user_api_key:
        ...
        try:
            ...
            # 仕送り状況のテキスト化
            boarding_status = "なし(自宅通学)"
            if c1_boarding: boarding_status = f"第1子あり(年{boarding_cost_yearly}万)"
            if c2_boarding: boarding_status += f", 第2子あり(年{boarding_cost_yearly}万)"
            ...
            # アドバイスの構成
            1. 家計の安全性診断 (A~E評価)
            2. 特に資金が厳しくなる時期(大学進学時など)への具体的な備え
            3. 住宅ローンと教育費のバランスについての指摘
            4. 投資・iDeCo活用の評価
            """
